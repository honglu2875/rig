...
Everything involved in training lives in this file.  The research gate targets
a TPU v4-32 and uses pure JAX: model state is replicated while the global
...
class Config:
    ...
    sparse_mlp_token_block: int
    sparse_mlp_output_block: int
    loss_backend: str
    vocab_tile_size: int
    dtype_name: str
...
class ModelDefinition:
    ...
    mlp_input_group_size: PositiveInt
    normalization: Literal["rms_norm"]
    position_encoding: Literal["rope_base_10000"]
    mlp_activation: Literal["double_fuzzy_topk_relu"]
    ...
    def validate(self, label: str) -> None:
        ...
        if self.d_model % self.mlp_input_group_size:
            raise ValueError(
                f"{label}.mlp_input_group_size must divide d_model"
            )
...
class TierDefinition:
    ...
    def stored_parameters(
        self, *, layers: int, mlp_mult: int, d_model: int | None = None
    ) -> int:
        """Stored parameters for one explicit depth/dictionary treatment."""

        model = self.model
        width = d_model or model.d_model
        return (
...
class KernelSettings:
    ...
    sparse_mlp_backend: Literal[
        "pallas_up", "pallas_up_dx", "choicewise", "reference"
    ]
...
class ExperimentConfig(ConfigSchema):
    ...
    def validate(self, label: str) -> None:
        ...
        if run.kernels.sparse_mlp_output_block % 128:
            raise ValueError(
                f"{label} run.kernels.sparse_mlp_output_block must be a "
                "multiple of 128"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sparse.add_argument(
        ...
        default=None,
        help=(
            "research override for model width; must preserve the tier's "
            "configured attention-head width"
        ),
    )
    sparse.add_argument(
        "--sparse-mlp-mult",
    ...
    sparse.add_argument(
        ...
        type=positive_int,
        default=None,
        help="input grouped-max size; 4 retains d_model / 4 coordinates",
    )
    sparse.add_argument(
        "--sparse-mlp-backend",
        choices=("pallas_up", "pallas_up_dx", "choicewise", "reference"),
        default=None,
        help=(
            "selected-row Pallas UP with bounded or selected-dX reverse, "
            "regular choice-wise, or gathered oracle"
        ),
    )
    sparse.add_argument(
        "--sparse-mlp-token-block",
        type=positive_int,
        default=None,
        help="token block for selected-row Pallas UP operations",
    )
    sparse.add_argument(
        "--sparse-mlp-output-block",
        type=positive_int,
        default=None,
        help="output block for selected-row Pallas UP operations",
    )
    sparse.add_argument(
...
def resolve_config(
    ...
    *,
    ...
    d_model = args.sparse_d_model or model.d_model
    head_dim = model.head_dim
    if d_model % head_dim:
        raise ValueError(
            f"--sparse-d-model {d_model} must be divisible by the tier's "
            f"{head_dim}-wide attention heads"
        )
    heads = d_model // head_dim
    mlp_mult = args.sparse_mlp_mult or model.mlp_mult
    mlp_top_k = args.sparse_top_k or model.mlp_top_k
    mlp_input_group_size = (
        args.sparse_input_group_size or model.mlp_input_group_size
    )
    if d_model % mlp_input_group_size:
        raise ValueError(
            f"--sparse-input-group-size {mlp_input_group_size} must divide "
            f"the resolved model width {d_model}"
        )
    hidden_width = mlp_mult * d_model
    ...
    sparse_mlp_token_block = (
        args.sparse_mlp_token_block or kernels.sparse_mlp_token_block
    )
    sparse_mlp_output_block = (
        args.sparse_mlp_output_block or kernels.sparse_mlp_output_block
    )
    if sparse_mlp_output_block % 128:
        raise ValueError("--sparse-mlp-output-block must be a multiple of 128")
    if (
        sparse_mlp_backend in ("pallas_up", "pallas_up_dx")
        and hidden_width % sparse_mlp_output_block
    ):
        raise ValueError(
            f"--sparse-mlp-output-block {sparse_mlp_output_block} must divide "
            f"hidden width {hidden_width} for pallas_up"
        )
    ...
    return Config(
        ...
        mlp_mult=mlp_mult,
        mlp_top_k=mlp_top_k,
        mlp_input_group_size=mlp_input_group_size,
...
def contract_model_metadata(config: Config) -> dict[str, Any]:
    ...
    return {
        ...
        "mlp_input_group_size": config.mlp_input_group_size,
...
def model_console_rows(
    ...
    return (
        (
            ...
            f"RoPE RMSNorm double-fuzzy-TopK-ReLU MLP×{config.mlp_mult} "
            f"input-group={config.mlp_input_group_size} k={config.mlp_top_k}",
        ),
...
def gpt_hidden(
    ...
    sparse_mlp_fn: DoubleFuzzyTopKCallable | None = None,
) -> jax.Array:
    ...
    sparse_mlp_config = DoubleFuzzyTopKConfig(
        top_k=config.mlp_top_k,
        input_group_size=config.mlp_input_group_size,
    ...
    for block in params["blocks"]:
        ...
        mlp_output = (
            ...
            else double_fuzzy_topk_mlp(*mlp_arguments, config=sparse_mlp_config)
        )
...
def double_fuzzy_topk_mlp_flop_rule(
    site: Site,
    *,
    config: Config,
    backend: str,
    ...
    """Bill either selected arithmetic or contractions physically issued.

    Let ``Q=D/input_group_size`` and ``K=mlp_top_k``. The architecture's
    selected arithmetic is ``2M(QH+KD)`` forward and ``4M(QK+KD)`` backward.
    The implementation count is intentionally backend-specific: regular
    masked contractions still cost their complete matrix shapes even where
    most logical entries are zero.
    """
    ...
    if active:
        if is_forward:
            return 2 * tokens * (
                input_top_k * hidden_width + hidden_top_k * model_width
            )
        return 4 * tokens * hidden_top_k * (input_top_k + model_width)

    if backend == "choicewise":
        multiplier = 4 if is_forward else 8
        return multiplier * tokens * model_width * hidden_width
    if backend == "pallas_up":
        if is_forward:
            return 2 * tokens * hidden_width * (input_top_k + model_width)
        return 8 * tokens * model_width * hidden_width
    if backend == "pallas_up_dx":
        if is_forward:
            return 2 * tokens * hidden_width * (input_top_k + model_width)
        padded_model_width = (
            math.ceil(model_width / config.sparse_mlp_output_block)
            * config.sparse_mlp_output_block
        )
        return 6 * tokens * model_width * hidden_width + 2 * (
            tokens * hidden_top_k * padded_model_width
        )
    if backend == "reference":
        ...
        return 2 * tokens * model_width * (
            3 * hidden_width + hidden_top_k
        )
    raise FlopError(f"unknown double fuzzy TopK backend {backend!r}")


def _double_fuzzy_flop_rules(config: Config, *, active: bool):
    rules = default_rules()
    for backend, scope_name in _DOUBLE_FUZZY_SCOPE_NAMES.items():
        rules = rules.with_scope(
            scope_name,
            lambda site, selected_backend=backend: double_fuzzy_topk_mlp_flop_rule(
                site,
                config=config,
...
def traced_flops(config: Config, params: Mapping[str, Any]) -> FlopBreakdown:
    ...
    * A component whose real cost differs from its traced cost -- sparsity
    ...
    return count_training_flops(
        loss,
        params,
        rules=_double_fuzzy_flop_rules(config, active=False),
    )


def active_traced_flops(
    config: Config, params: Mapping[str, Any]
) -> FlopBreakdown:
    """Count ideal selected matrix arithmetic for scientific matching.

    This uses the identical whole-model trace as :func:`traced_flops`, but
    replaces the MLP boundary by the multiplies whose operands are logically
    active. It does not claim the selected arithmetic is what a given backend
    physically issues.
    """
    ...
    return count_training_flops(
        loss,
        params,
        rules=_double_fuzzy_flop_rules(config, active=True),
    )
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    active_flop_breakdown = active_traced_flops(config, host_params)
    active_flops_per_token = active_flop_breakdown.per_token(config.seq_len)
    for warning in flop_breakdown.warnings:
        console.warn(f"FLOP accounting: {warning}")
    for warning in active_flop_breakdown.warnings:
        console.warn(f"active FLOP accounting: {warning}")
    ...
    sparse_mlp_fn = make_mesh_double_fuzzy_topk_mlp(
    ...
    result: dict[str, Any] = {
        ...
        "contract": {
            "model_id": "double-fuzzy-topk-autoencoder-gpt-v1-family",
        ...
        "metrics": {
            ...
            "active_flops_per_token": int(active_flops_per_token),
            "estimated_active_total_flops": int(
                active_flops_per_token * tokens_processed
            ),
            ...
            "active_flop_accounting": {
                "method": "traced-jaxpr-selected-matrix-arithmetic",
                "matmul_per_sequence": int(active_flop_breakdown.matmul),
                "elementwise_per_sequence": int(
                    active_flop_breakdown.elementwise
                ),
                "by_site": {
                    label: int(value)
                    for label, value in sorted(
                        active_flop_breakdown.by_site.items()
                    )
                },
                "warnings": list(active_flop_breakdown.warnings),
            },
