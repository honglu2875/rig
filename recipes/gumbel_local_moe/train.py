...
class Config:
    ...
    # Additional stateless local updates after the ordinary AdamW step.
    local_moe_steps: int = 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    optim.add_argument(
        ...
        default=2,
        help=(
            "additional stateless MoE-local updates per global AdamW step; "
            "zero is the exact reference optimizer path"
        ),
    )
...
def resolve_config(
    ...
    *,
    ...
    return Config(
        ...
        local_moe_steps=args.local_moe_steps,
...
def model_console_rows(
    ...
    return (
        ...
        (
            ...
            f"Gumbel-local steps {config.local_moe_steps}",
        ),
    )
...
def implementation_metadata(
    ...
    return {
        ...
        "local_moe_optimization": {
            "steps": config.local_moe_steps,
            "routing": "standard_gumbel_top_k_selection_clean_gate_weights",
            "objective": "variance_normalized_output_delta_plus_output_gradient",
            "statistics": "per_layer_ema_input_output_output_gradient_second_moment",
            "statistics_decay": "effective_adam_beta2",
            "inner_optimizer": "stateless_sgd_existing_learning_rate_no_decay",
        },
        "context_preset": config.context_preset,
        "document_masking": config.document_masking,
        "configuration": experiment_config_metadata(config),
...
@jax.tree_util.register_dataclass
@dataclass(frozen=True)
class MoELocalActivations:
    """Frozen outer-pass values used by the additional MoE-local updates."""

    # All arrays are [layers, batch, sequence, width]. ``normalized_inputs``
    # feed the routed MLP; residual_inputs and outputs define the complete
    # residual sublayer whose input/output scale supplies the local metric.
    normalized_inputs: jax.Array
    residual_inputs: jax.Array
    outputs: jax.Array
...
def select_top_k_routes(
    ...
    *,
    ...
    """Choose routes, optionally with Gumbel exploration, and return clean gates."""

    selection_logits = logits
    if router_key is not None:
        # Gumbel-Max is the categorical sampler: the discrete top-k is the
        # desired operation, not a relaxation. Fold in the data-axis ordinal
        # so replicated keys still produce independent samples on each shard.
        if axis_name is not None:
            router_key = jax.random.fold_in(router_key, jax.lax.axis_index(axis_name))
        selection_logits = logits + jax.random.gumbel(
            router_key, logits.shape, dtype=jnp.float32
        )
    _, chosen = jax.lax.top_k(selection_logits, top_k)
    # Noise chooses the experts only. Clean logits retain the reference gate
    # amplitudes, isolating assignment exploration from mixture-weight noise.
    chosen_logits = jnp.take_along_axis(logits, chosen, axis=-1)
...
def routed_mlp_local(
    ...
    *,
    ...
    chosen, gate = select_top_k_routes(logits, top_k, router_key, axis_name=axis_name)
...
def make_mesh_gumbel_routed_mlp(config: Config, mesh: Mesh) -> Any:
    """Return the same routed kernel with a replicated Gumbel PRNG input."""
...
def gpt_hidden(
    ...
    routed_fn: Any = None,
    moe_perturbations: jax.Array | None = None,
    capture_moe_activations: bool = False,
) -> tuple[jax.Array, "RouterStats | None", "MoELocalActivations | None"]:
    """Return final token representations, and router statistics when routed."""

    dtype = config.compute_dtype
    batch, length = tokens.shape
    if moe_perturbations is not None:
        expected = (config.layers, batch, length, config.d_model)
        if moe_perturbations.shape != expected:
            raise ValueError(
                f"MoE perturbations must have shape {expected}; "
                f"got {moe_perturbations.shape}"
            )
    if capture_moe_activations and not config.experts:
        raise ValueError("MoE-local activation capture requires routed blocks")
    ...
    local_normalized_inputs: list[jax.Array] = []
    local_residual_inputs: list[jax.Array] = []
    local_outputs: list[jax.Array] = []
    ...
    for layer, block in enumerate(params["blocks"]):
        ...
        if config.experts:
            if capture_moe_activations:
                local_normalized_inputs.append(x_norm)
                local_residual_inputs.append(residual)
        ...
        if config.experts and capture_moe_activations:
            local_outputs.append(x)
        if moe_perturbations is not None:
            x = x + moe_perturbations[layer].astype(x.dtype)
    ...
    local_activations = (
        MoELocalActivations(
            normalized_inputs=jnp.stack(local_normalized_inputs),
            residual_inputs=jnp.stack(local_residual_inputs),
            outputs=jnp.stack(local_outputs),
        )
        if capture_moe_activations
        else None
    )
    return hidden_state, router, local_activations
...
def gpt_logits_and_router(
    ...
    x, router, _ = gpt_hidden(params, tokens, config, attention_fn, routed_fn)
...
def cross_entropy(
    ...
    if config.loss_backend == "tiled":
        hidden, router, _ = gpt_hidden(params, x, config, attention_fn, routed_fn)
...
def cross_entropy_router_and_local_activations(
    params: Mapping[str, Any],
    ...
    attention_fn: AttentionCallable | None = None,
    routed_fn: Any = None,
) -> tuple[jax.Array, tuple["RouterStats", "MoELocalActivations"]]:
    """Outer objective with the MoE residual outputs exposed to autodiff.

    The perturbations are identically zero. Their gradients are therefore the
    output cotangents of every MoE residual sublayer from the ordinary global
    backward pass, obtained without a second model traversal.
    """

    hidden, router, activations = gpt_hidden(
        params,
        x,
        config,
        attention_fn,
        routed_fn,
        moe_perturbations=perturbations,
        capture_moe_activations=True,
    )
    if config.loss_backend == "tiled":
        loss = tiled_tied_cross_entropy(
            hidden,
    ...
    if router is None or activations is None:
        raise AssertionError("MoE-local objective did not capture routed activations")
    if config.router_aux_coefficient:
        loss = loss + config.router_aux_coefficient * router.balance_loss
    return loss, (router, activations)
...
def init_optimizer(params: Any, config: Config, seed: int = 0) -> dict[str, Any]:
    ...
    history = np.zeros(
        (
            config.steps,
            3 + router_row_width(config) + local_moe_row_width(config),
        ),
        dtype=np.float32,
    )
    return {
        ...
        "v": zeros,
        "history": history,
        # Three per-layer EMAs: residual input, residual output, and output
        # gradient second moments. They are updated only for treatment runs.
        "local_moments": np.zeros((config.layers, 3), dtype=np.float32),
        "local_rng": np.asarray(jax.random.PRNGKey(seed), dtype=np.uint32),
    }
...
def moe_parameter_tree(params: Mapping[str, Any]) -> list[dict[str, jax.Array]]:
    """Extract only parameters read by a routed MLP local objective."""

    return [
        {name: block[name] for name in _MOE_PARAMETER_NAMES}
        for block in params["blocks"]
    ]


def replace_moe_parameter_tree(
    params: Mapping[str, Any], moe_params: Sequence[Mapping[str, jax.Array]]
) -> dict[str, Any]:
    """Return ``params`` with only router/expert leaves replaced."""

    if len(moe_params) != len(params["blocks"]):
        raise ValueError("local MoE parameters must contain one entry per block")
    blocks = [
        {**block, **dict(local)}
        for block, local in zip(params["blocks"], moe_params, strict=True)
    ]
    return {**params, "blocks": blocks}


def moe_local_second_moments(
    activations: MoELocalActivations, output_gradients: jax.Array
) -> jax.Array:
    """Return [layer, input/output/output-gradient] mean squares."""

    axes = (1, 2, 3)

    def mean_square(value: jax.Array) -> jax.Array:
        value = value.astype(jnp.float32)
        return jnp.mean(jnp.square(value), axis=axes)

    return jnp.stack(
        (
            mean_square(activations.residual_inputs),
            mean_square(activations.outputs),
            mean_square(output_gradients),
        ),
        axis=1,
    )


def update_moe_local_moments(
    previous: jax.Array,
    current: jax.Array,
    *,
    beta2: float,
    ...
    """Update and bias-correct the three scalar EMAs used by each layer."""

    moments = beta2 * previous + (1.0 - beta2) * current
    correction = 1.0 - beta2 ** step.astype(jnp.float32)
    return moments, moments / correction


def moe_local_objective(
    moe_params: Sequence[Mapping[str, jax.Array]],
    activations: MoELocalActivations,
    output_gradients: jax.Array,
    corrected_moments: jax.Array,
    router_key: jax.Array,
    config: Config,
    gumbel_routed_fn: Any = None,
) -> tuple[jax.Array, jax.Array]:
    """Variance-normalized local quadratic under sampled Gumbel-top-k routes.

    For each layer this is ``0.5 * mean((delta / s + g / q) ** 2)``.
    The cross term is the first-order global loss change; the squared output
    displacement is the activation-space restraint. Input/output/gradient
    scales are stop-gradient outer-pass statistics.
    """

    routed = gumbel_routed_fn or functools.partial(
        routed_mlp_local,
        experts=config.experts,
        top_k=config.expert_top_k,
        dtype=config.compute_dtype,
    ...
    keys = jax.random.split(router_key, config.layers)
    residual_scale = config.depth_multiplier ** (-config.depth_alpha)
    tiny = jnp.asarray(jnp.finfo(jnp.float32).tiny, jnp.float32)
    layer_losses: list[jax.Array] = []

    for layer, local_params in enumerate(moe_params):
        mlp_out, _, _, _ = routed(
            activations.normalized_inputs[layer],
            local_params["router_w"],
            local_params["expert_up_w"],
            local_params["expert_up_b"],
            local_params["expert_down_w"],
            local_params["expert_down_b"],
            keys[layer],
        )
        candidate = (
            activations.residual_inputs[layer] + residual_scale * mlp_out
        ).astype(jnp.float32)
        delta = candidate - activations.outputs[layer].astype(jnp.float32)
        activation_second_moment = 0.5 * (
            corrected_moments[layer, 0] + corrected_moments[layer, 1]
        )
        activation_rms = jnp.sqrt(jnp.maximum(activation_second_moment, tiny))
        gradient_rms = jnp.sqrt(jnp.maximum(corrected_moments[layer, 2], tiny))
        normalized = (
            delta / activation_rms
            + output_gradients[layer].astype(jnp.float32) / gradient_rms
        )
        layer_losses.append(0.5 * jnp.mean(jnp.square(normalized), dtype=jnp.float32))

    per_layer = jnp.stack(layer_losses)
    return jnp.sum(per_layer), per_layer


def apply_moe_local_updates(
    params: Mapping[str, Any],
    ...
    *,
    ...
    lr_multipliers: Any,
    gumbel_routed_fn: Any = None,
) -> tuple[dict[str, Any], jax.Array, jax.Array, jax.Array, jax.Array]:
    """Take fixed-count stateless SGD steps on the local MoE objective."""

    _, beta2 = effective_adam_betas(config)
    current_moments = moe_local_second_moments(activations, output_gradients)
    moments, corrected = update_moe_local_moments(
        optimizer["local_moments"], current_moments, beta2=beta2, step=step
    )
    local_params = moe_parameter_tree(params)
    local_lr_multipliers = moe_parameter_tree({"blocks": lr_multipliers["blocks"]})
    rng = optimizer["local_rng"]
    layer_losses = jnp.zeros((config.layers,), jnp.float32)

    for _ in range(config.local_moe_steps):
        rng, router_key = jax.random.split(rng)
        objective = functools.partial(
            moe_local_objective,
            activations=activations,
            output_gradients=output_gradients,
            corrected_moments=corrected,
            router_key=router_key,
            config=config,
            gumbel_routed_fn=gumbel_routed_fn,
        )
        (local_loss, layer_losses), local_gradients = jax.value_and_grad(
            objective, has_aux=True
        )(local_params)
        del local_loss
        local_params = jax.tree_util.tree_map(
            lambda parameter, gradient, multiplier: (
                parameter - lr * multiplier * gradient.astype(jnp.float32)
            ),
            local_params,
            local_gradients,
            local_lr_multipliers,
        )

    return (
        replace_moe_parameter_tree(params, local_params),
        moments,
        rng,
        layer_losses,
        corrected,
...
def _apply_training_update(
    ...
    beta1, beta2 = effective_adam_betas(config)
    local_activations: MoELocalActivations | None = None
    output_gradients: jax.Array | None = None
    if config.local_moe_steps:
        zero_token = jnp.zeros_like(x, dtype=config.compute_dtype)[..., None]
        perturbations = jnp.broadcast_to(
            zero_token,
            (config.layers, x.shape[0], x.shape[1], config.d_model),
        )
        ((loss, (router_aux, local_activations)), (gradients, output_gradients)) = (
            jax.value_and_grad(
                lambda candidate, delta: cross_entropy_router_and_local_activations(
                    candidate,
                    delta,
                    x,
                    y,
    ...
    local_moments = optimizer["local_moments"]
    local_rng = optimizer["local_rng"]
    local_layer_losses = jnp.zeros((0,), jnp.float32)
    corrected_local_moments = jnp.zeros((0, 3), jnp.float32)
    if config.local_moe_steps:
        if local_activations is None or output_gradients is None:
            raise AssertionError("local MoE update is missing outer-pass values")
        (
            params,
            local_moments,
            local_rng,
            local_layer_losses,
            corrected_local_moments,
        ) = apply_moe_local_updates(
            params,
            optimizer,
            local_activations,
            output_gradients,
    ...
    local_metrics = local_moe_row(config, local_layer_losses, corrected_local_moments)
    ...
    return (
        ...
        },
        {
            ...
            "router_row": routing,
            "local_layer_losses": local_layer_losses,
            "local_moments": corrected_local_moments,
            "local_row": local_metrics,
        },
        raw_gradients,
    )
...
def local_moe_row(
    config: Config, layer_losses: jax.Array, corrected_moments: jax.Array
) -> jax.Array:
    """Flatten the four local-objective scalars recorded for every block."""

    if not config.local_moe_steps:
        return jnp.zeros((0,), jnp.float32)
    if layer_losses.shape != (config.layers,) or corrected_moments.shape != (
        config.layers,
        3,
    ):
        raise ValueError(
            "local MoE metrics require one loss and three moments per block"
        )
    rms = jnp.sqrt(jnp.maximum(corrected_moments, 0.0))
    return jnp.stack(
        (
            layer_losses,
            rms[:, 0],
...
def local_moe_row_width(config: Config) -> int:
    return config.layers * len(_LOCAL_MOE_METRICS) if config.local_moe_steps else 0


def recipe_training_log_columns(config: Config) -> tuple[logpack.Column, ...]:
    """Return reference columns plus treatment-only per-block scalars."""

    columns = list(training_log_columns(config.layers, config.experts))
    if config.local_moe_steps:
        for layer in range(config.layers):
            columns.extend(
                logpack.column(name, "block", layer) for name in _LOCAL_MOE_METRICS
            )
...
def eval_step(
    ...
    if config.loss_backend == "tiled":
        hidden, _, _ = gpt_hidden(params, x, config, attention_fn, routed_fn)
...
def traced_flops(config: Config, params: Mapping[str, Any]) -> FlopBreakdown:
    ...
    * A component whose real cost differs from its traced cost -- sparsity
    ...
    rules = default_rules()
    outer = count_training_flops(loss, params, rules=rules)
    if not config.local_moe_steps:
        return outer

    local_shape = (config.layers, 1, config.seq_len, config.d_model)
    activation_spec = jax.ShapeDtypeStruct(local_shape, config.compute_dtype)
    gradient_spec = jax.ShapeDtypeStruct(local_shape, jnp.float32)
    corrected_moments = jnp.ones((config.layers, 3), jnp.float32)
    router_key = jax.random.PRNGKey(0)

    def local_loss(
        trainable: Sequence[Mapping[str, jax.Array]],
        normalized_inputs: jax.Array,
        residual_inputs: jax.Array,
        outputs: jax.Array,
        output_gradients: jax.Array,
    ...
    local = count_training_flops(
        local_loss,
        moe_parameter_tree(params),
        activation_spec,
        activation_spec,
        activation_spec,
        gradient_spec,
    ...
    multiplier = config.local_moe_steps
    labels = set(outer.by_site) | set(local.by_site)
    return FlopBreakdown(
        matmul=outer.matmul + multiplier * local.matmul,
        elementwise=outer.elementwise + multiplier * local.elementwise,
        by_site={
            label: outer.by_site.get(label, 0)
            + multiplier * local.by_site.get(label, 0)
            for label in labels
        },
        warnings=tuple(dict.fromkeys((*outer.warnings, *local.warnings))),
    )
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    host_optimizer = init_optimizer(host_params, config, args.seed)
    ...
    gumbel_routed_fn = make_mesh_gumbel_routed_mlp(config, mesh)
    ...
    training_columns = recipe_training_log_columns(config)
                ...
                if should_log:
                    ...
                    append_log_row(
                        ...
                        (
                            ...
                            *(float(v) for v in host_metrics["local_row"]),
                        ),
                    )
                    console.step(
                        step_index,
    ...
    local_moe_summary: dict[str, int | float] = {
        "steps_per_global_update": int(config.local_moe_steps)
    }
    if config.local_moe_steps:
        final_local_losses = np.asarray(
            final_train["local_layer_losses"], dtype=np.float32
        )
        final_local_rms = np.sqrt(
            np.maximum(
                np.asarray(final_train["local_moments"], dtype=np.float32),
                0.0,
            )
        )
        local_moe_summary.update(
            {
                "final_mean_objective": finite_metric(
                    "final_mean_local_moe_objective",
                    float(final_local_losses.mean()),
                ),
                "final_mean_input_rms": finite_metric(
                    "final_mean_local_moe_input_rms",
                    float(final_local_rms[:, 0].mean()),
                    positive=True,
                ),
                "final_mean_output_rms": finite_metric(
                    "final_mean_local_moe_output_rms",
                    float(final_local_rms[:, 1].mean()),
                    positive=True,
                ),
                "final_mean_output_gradient_rms": finite_metric(
                    "final_mean_local_moe_output_gradient_rms",
                    float(final_local_rms[:, 2].mean()),
                    positive=True,
                ),
            }
        )
