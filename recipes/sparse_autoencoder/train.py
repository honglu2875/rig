...
class ModelDefinition:
    ...
    mlp_activation: Literal["topk_relu"]
...
class ExperimentConfig(ConfigSchema):
    ...
    def validate(self, label: str) -> None:
        ...
        sparse_kernels = run.kernels
        if (
            sparse_kernels.sparse_mlp_backend == "pallas"
            and sparse_kernels.sparse_mlp_output_block % 128
        ):
            raise ValueError(
                f"{label} run.kernels.sparse_mlp_output_block must be a "
                "multiple of 128 for Pallas"
            )
        if sparse_kernels.sparse_mlp_backend == "pallas":
            for tier_name, tier in family.tiers.items():
                if tier.model.d_model % sparse_kernels.sparse_mlp_output_block:
                    raise ValueError(
                        f"{label} family.tiers.{tier_name}.model.d_model must be "
...
def resolve_config(
    ...
    *,
    ...
    if sparse_mlp_backend == "pallas" and platform != "tpu":
        raise ValueError(
    ...
    if (
        ...
        and kernels.sparse_mlp_token_block * mlp_top_k > 65_536
    ):
        raise ValueError(
...
def model_console_rows(
    ...
    return (
        (
            ...
            f"RoPE RMSNorm TopK-ReLU MLP×{config.mlp_mult} k={config.mlp_top_k}",
        ),
...
def gpt_hidden(
    ...
    sparse_mlp_config = SparseMlpConfig(
...
def sparse_topk_mlp_flop_rule(site: Site) -> int:
    """Bill the dense scoring pass and only the selected decoder work.

    The named JIT appears once for the forward rule (five inputs) and once for
    its custom VJP (the same five values, two TopK residuals, and dOutput).
    Forward costs ``2 M D H + 2 M K D``.  The VJP has four sparse contractions:
    dValues, dX, dW_up, and dW_down, each ``2 M K D``.
    """
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    result: dict[str, Any] = {
        ...
        "contract": {
            "model_id": "sparse-autoencoder-gpt-v1-family",
