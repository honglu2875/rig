...
class Config:
    ...
    def balance_enabled(self) -> bool:
        """Whether training adds either differentiable homeostasis objective."""

        return self.balance_coefficient > 0.0 or self.alive_coefficient > 0.0

    @property
    def soft_balance_enabled(self) -> bool:
        return self.balance_mode != "none" and self.balance_coefficient > 0.0

    @property
    def auxk_enabled(self) -> bool:
        return self.auxk_mode == "ghost" and self.auxk_coefficient > 0.0
...
@dataclass(frozen=True, slots=True)
class AuxKSettings:
    """Paper-inspired initialization and dead-feature ghost path."""

    init_mode: Literal["independent", "tied_directions"]
    mode: Literal["none", "ghost"]
    coefficient: NonnegativeFloat
    ...
    def validate(self, label: str) -> None:
        if self.mode == "none" and self.coefficient != 0.0:
            raise ValueError(f"{label}.coefficient must be zero when mode is none")
        if self.mode == "ghost" and self.coefficient == 0.0:
            raise ValueError(f"{label}.coefficient must be positive in ghost mode")
...
class ExperimentConfig(ConfigSchema):
    ...
    def validate(self, label: str) -> None:
        ...
        family = self.family
        self.run.balance.validate(f"{label} run.balance")
        self.run.auxk.validate(f"{label} run.auxk")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sparse.add_argument(
        ...
        default=None,
        help="coefficient on the selected within-group balance objective",
    )
    sparse.add_argument(
        "--fuzzy-balance-temperature",
        type=positive_float,
        default=None,
        help="temperature for the differentiable four-choice probabilities",
    )
    sparse.add_argument(
    ...
    sparse.add_argument(
        "--fuzzy-init-mode",
        choices=("independent", "tied_directions"),
        default=None,
        help="independent MLP matrices or paper-style tied directions at initialization",
    )
    sparse.add_argument(
        "--fuzzy-auxk-mode",
        choices=("none", "ghost"),
        default=None,
        help="disable or train recently dead features through a zero-forward ghost path",
    )
    sparse.add_argument(
        "--fuzzy-auxk-coefficient",
        type=nonnegative_float,
        default=None,
        help="reverse-only ghost-path coefficient (the paper default is 1/32)",
    )
    sparse.add_argument(
        "--fuzzy-auxk-width-ratio",
        type=positive_float,
        default=None,
        help="eligible auxiliary winners per step divided by d_model",
    )
    sparse.add_argument(
        "--fuzzy-dead-tokens-threshold",
        type=positive_int,
        default=None,
        help="tokens without a main activation before a feature becomes dead",
    )
    sparse.add_argument(
...
def should_run_sparsity_diagnostics(step: int, *, every: int, final_step: int) -> bool:
...
def resolve_config(
    ...
    *,
    ...
    balance_mode = args.fuzzy_balance_mode or balance.mode
    balance_coefficient = (
        balance.coefficient
        if args.fuzzy_balance_coefficient is None
        else args.fuzzy_balance_coefficient
    )
    balance_temperature = (
        balance.temperature
        if args.fuzzy_balance_temperature is None
        else args.fuzzy_balance_temperature
    )
    alive_coefficient = (
        balance.alive_coefficient
    ...
    init_mode = args.fuzzy_init_mode or auxk.init_mode
    ...
    if balance_mode != "none" and balance_coefficient == 0.0:
        raise ValueError(
            "--fuzzy-balance-coefficient must be positive when balance is enabled"
        )
    if (
        balance_coefficient > 0.0 or alive_coefficient > 0.0
    ) and sparse_mlp_backend != "choicewise":
        raise ValueError("fuzzy balance objectives require the choicewise backend")
    if auxk_mode == "none" and auxk_coefficient != 0.0:
        raise ValueError("--fuzzy-auxk-coefficient must be zero when AuxK mode is none")
    if auxk_mode == "ghost" and auxk_coefficient == 0.0:
        raise ValueError("--fuzzy-auxk-coefficient must be positive in ghost mode")
    ...
    if auxk_mode == "ghost" and sparse_mlp_backend != "choicewise":
        raise ValueError("the fuzzy AuxK ghost path requires the choicewise backend")
    if auxk_mode == "ghost" and (
        balance_coefficient > 0.0 or alive_coefficient > 0.0
    ):
        raise ValueError("AuxK and frequency-balance treatments are separate study arms")
...
def init_params(config: Config, seed: int) -> dict[str, Any]:
    ...
    for _ in range(config.layers):
        ...
        independent_down_w = normal(rng, (hidden, d_model), hidden_scale)
        mlp_down_w = (
            mlp_up_w.T.copy()
            if config.init_mode == "tied_directions"
            else independent_down_w
        )
        blocks.append(
            {
...
def model_console_rows(
    ...
    balance_description = (
        f"{config.balance_mode}×{config.balance_coefficient:g}"
    ...
    auxk_description = (
        f"ghost k_aux={config.aux_k} × {config.auxk_coefficient:g}; "
        f"dead after {config.dead_tokens_threshold:,} tokens"
    ...
    return (
        ...
        (
            "fuzzy homeostasis",
            f"balance {balance_description} · alive {alive_description}",
        ),
        ("paper-inspired initialization", config.init_mode),
        ("paper-inspired AuxK", auxk_description),
    )
...
def implementation_metadata(
    ...
    return {
        "attention_backend": config.attention_backend,
        "sparse_mlp_backend": config.sparse_mlp_backend,
        "fuzzy_balance_mode": config.balance_mode,
        ...
        "fuzzy_init_mode": config.init_mode,
        "fuzzy_auxk_mode": config.auxk_mode,
        "fuzzy_auxk_coefficient": config.auxk_coefficient,
        "fuzzy_auxk_width_ratio": config.auxk_width_ratio,
        "fuzzy_auxk": config.aux_k,
        "fuzzy_dead_tokens_threshold": config.dead_tokens_threshold,
        "fuzzy_dead_after_steps": config.dead_after_steps,
...
class FuzzyBalanceStats:
    """Per-block objectives carried out of value_and_grad for logging."""

    # [layers, 3], ordered exactly as FUZZY_BALANCE_STAT_NAMES.
    layers: jax.Array


@jax.tree_util.register_dataclass
@dataclass(frozen=True)
class FuzzyAuxKStats:
    """Global main-path activation counts used to update dead-feature ages."""
...
@jax.tree_util.register_dataclass
@dataclass(frozen=True)
class FuzzyTrainingStats:
    """Optional training-only statistics carried out of value_and_grad."""

    balance: FuzzyBalanceStats | None
    auxk: FuzzyAuxKStats | None
...
def gpt_hidden(
    ...
    balanced_mlp_fn: FuzzyTopKBalanceCallable | None = None,
    auxk_mlp_fn: FuzzyTopKAuxKCallable | None = None,
    dead_mask: jax.Array | None = None,
    auxk_cohort: jax.Array | None = None,
) -> (
    jax.Array
    | tuple[jax.Array, jax.Array]
    | tuple[jax.Array, FuzzyBalanceStats]
    | tuple[jax.Array, FuzzyAuxKStats]
):
    ...
    if supplied > 1:
        raise ValueError(
            "ordinary, diagnostic, balanced, and AuxK MLP callables are exclusive"
        )
    if auxk_mlp_fn is not None and (dead_mask is None or auxk_cohort is None):
        raise ValueError("AuxK MLP requires a dead mask and cohort")
    ...
    balance_statistics: list[jax.Array] = []
    auxk_statistics: list[jax.Array] = []
    ...
    for layer_index, block in enumerate(params["blocks"]):
        ...
        elif balanced_mlp_fn is not None:
            mlp_output, layer_balance = balanced_mlp_fn(*mlp_arguments)
            balance_statistics.append(layer_balance)
        elif auxk_mlp_fn is not None:
            assert dead_mask is not None and auxk_cohort is not None
            mlp_output, layer_active_counts = auxk_mlp_fn(
                *mlp_arguments,
                dead_mask[layer_index],
                auxk_cohort,
            )
            auxk_statistics.append(layer_active_counts)
        else:
            mlp_output = (
    ...
    if balanced_mlp_fn is not None:
        return hidden, FuzzyBalanceStats(layers=jnp.stack(balance_statistics))
    if auxk_mlp_fn is not None:
        return hidden, FuzzyAuxKStats(active_counts=jnp.stack(auxk_statistics))
    return hidden
...
def cross_entropy_and_balance(
    ...
    dead_mask: jax.Array | None = None,
    auxk_cohort: jax.Array | None = None,
) -> tuple[jax.Array, tuple[jax.Array, FuzzyTrainingStats]]:
    """Training objective plus unpolluted CE and training-only statistics."""

    if not config.balance_enabled and not config.auxk_enabled:
        cross_entropy_loss = cross_entropy(
            params, x, y, config, attention_fn, sparse_mlp_fn
        )
        return cross_entropy_loss, (
            cross_entropy_loss,
            FuzzyTrainingStats(balance=None, auxk=None),
        )

    balance: FuzzyBalanceStats | None = None
    auxk: FuzzyAuxKStats | None = None
    if config.balance_enabled:
        if balanced_mlp_fn is None:
            raise ValueError("enabled fuzzy homeostasis has no balanced MLP callable")
        hidden, returned = gpt_hidden(
            params,
            x,
            config,
            attention_fn,
            balanced_mlp_fn=balanced_mlp_fn,
        )
        if not isinstance(returned, FuzzyBalanceStats):
            raise AssertionError("balanced MLP did not return its objective statistics")
        balance = returned
    else:
        if auxk_mlp_fn is None or dead_mask is None or auxk_cohort is None:
            raise ValueError("enabled fuzzy AuxK has no ghost callable or dead state")
        hidden, returned = gpt_hidden(
            params,
            x,
            config,
            attention_fn,
            auxk_mlp_fn=auxk_mlp_fn,
            dead_mask=dead_mask,
            auxk_cohort=auxk_cohort,
        )
        if not isinstance(returned, FuzzyAuxKStats):
            raise AssertionError("AuxK MLP did not return activation counts")
        auxk = returned
    if config.loss_backend == "tiled":
    ...
    if config.alive_coefficient:
        assert balance is not None
        objective = objective + config.alive_coefficient * jnp.mean(
            balance.layers[:, 2]
        )
    return objective, (
        cross_entropy_loss,
        FuzzyTrainingStats(balance=balance, auxk=auxk),
    )
...
def balance_row(balance: FuzzyBalanceStats | None) -> jax.Array:
    """Flatten model-wide then per-block balance objectives for the run log."""

    if balance is None:
        return jnp.zeros((0,), jnp.float32)
    layers = balance.layers.astype(jnp.float32)
    return jnp.concatenate((jnp.mean(layers, axis=0), layers.reshape(-1)))


def balance_row_width(layers: int) -> int:
    return 0 if not layers else len(FUZZY_BALANCE_STAT_NAMES) * (layers + 1)


def fuzzy_training_log_columns(config: Config) -> tuple[logpack.Column, ...]:
    columns = list(training_log_columns())
    if config.balance_enabled:
        columns.extend(logpack.column(name) for name in FUZZY_BALANCE_STAT_NAMES)
        for layer in range(config.layers):
            columns.extend(
                logpack.column(name, "block", layer)
                for name in FUZZY_BALANCE_STAT_NAMES
            )
    if config.auxk_enabled:
        columns.extend(logpack.column(name) for name in AUXK_STAT_NAMES)
...
def init_optimizer(
    ...
    steps: int,
    *,
    balance_layers: int = 0,
    auxk_shape: tuple[int, int] | None = None,
) -> dict[str, Any]:
    ...
    history = np.zeros(
        (
            steps,
            3
            + balance_row_width(balance_layers)
            + (len(AUXK_STAT_NAMES) if auxk_shape is not None else 0),
        ),
        dtype=np.float32,
    )
    result = {
    ...
    if auxk_shape is not None:
        result["dead_steps"] = np.zeros(auxk_shape, dtype=np.int32)
    return result
...
def _apply_training_update(
    ...
    if config.auxk_enabled:
        if "dead_steps" not in optimizer:
            raise ValueError("enabled AuxK training has no dead-feature state")
        dead_mask = optimizer["dead_steps"] >= jnp.asarray(
            config.dead_after_steps, jnp.int32
        )
        auxk_cohort = jnp.mod(
            optimizer["step"],
            jnp.asarray(config.mlp_top_k // config.aux_k, jnp.int32),
        )
    else:
        dead_mask = None
        auxk_cohort = None
    (objective_and_aux, gradients) = jax.value_and_grad(
        lambda candidate: cross_entropy_and_balance(
            candidate,
            x,
            y,
            config,
    ...
    objective, (loss, training_stats) = objective_and_aux
    ...
    balance_values = balance_row(training_stats.balance)
    if training_stats.auxk is not None:
        active = training_stats.auxk.active_counts > 0.0
        ...
        dead_steps = None
        auxk_values = jnp.zeros((0,), jnp.float32)
    ...
    return (
        ...
        next_optimizer,
        {
            ...
            "grad_norm": grad_norm,
            "learning_rate": lr,
            "balance_row": balance_values,
            "auxk_row": auxk_values,
        },
        raw_gradients,
    )
...
def balanced_fuzzy_topk_mlp_flop_rule(site: Site) -> int:
    """Bill shared scoring plus one score recomputation in the auxiliary VJP."""

    if len(site.in_shapes) < 5:
        raise FlopError(f"unexpected balanced fuzzy boundary shapes: {site.in_shapes}")
    x_shape = site.in_shapes[0]
    up_shape = site.in_shapes[1]
    if len(x_shape) < 2 or len(up_shape) != 2 or up_shape[0] != x_shape[-1]:
        raise FlopError(f"unexpected balanced fuzzy operands: {site.in_shapes}")
    tokens = math.prod(x_shape[:-1])
    model_width, hidden_width = up_shape
    # Forward is the same 4MDH as choicewise fuzzy Top-K. Reverse is its
    # ordinary 8MDH plus a 2MDH score recomputation, for 14MDH end to end.
    multiplier = 4 if len(site.in_shapes) == 5 else 10
...
def auxk_fuzzy_topk_mlp_flop_rule(site: Site, *, cohort_count: int) -> int:
    """Bill parent work plus the reverse-only auxiliary cohort contractions."""

    if len(site.in_shapes) < 7:
        raise FlopError(f"unexpected fuzzy AuxK boundary shapes: {site.in_shapes}")
    x_shape = site.in_shapes[0]
    up_shape = site.in_shapes[1]
    if len(x_shape) < 2 or len(up_shape) != 2 or up_shape[0] != x_shape[-1]:
        raise FlopError(f"unexpected fuzzy AuxK operands: {site.in_shapes}")
    tokens = math.prod(x_shape[:-1])
    model_width, hidden_width = up_shape
    if len(site.in_shapes) == 7:
        return 4 * tokens * model_width * hidden_width
    main_reverse = 8 * tokens * model_width * hidden_width
    ghost_reverse = (
        6 * tokens * model_width * hidden_width // cohort_count
    )
    return main_reverse + ghost_reverse
...
def traced_flops(config: Config, params: Mapping[str, Any]) -> FlopBreakdown:
    ...
    * A component whose real cost differs from its traced cost -- sparsity
    ...
    auxk_config = FuzzyTopKAuxKConfig(
        top_k=config.mlp_top_k,
        aux_k=config.aux_k,
        coefficient=config.auxk_coefficient,
    )
    ...
    def loss(trainable: Mapping[str, Any]) -> jax.Array:
        ...
        if config.auxk_enabled:
            ...
            objective, _ = cross_entropy_and_balance(
                trainable,
                tokens,
                targets,
                config,
                auxk_mlp_fn=functools.partial(
                    fuzzy_topk_mlp_with_auxk, config=auxk_config
                ),
                dead_mask=dead_mask,
        ...
        objective, _ = cross_entropy_and_balance(
            ...
            targets,
            config,
            balanced_mlp_fn=functools.partial(
    ...
    rules = (
        ...
        .with_scope(
            "_choicewise_fuzzy_topk_auxk_mlp",
            functools.partial(
                auxk_fuzzy_topk_mlp_flop_rule,
                cohort_count=config.mlp_top_k // config.aux_k,
            ),
        )
    )
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    host_optimizer = init_optimizer(
        host_params,
        config.steps,
        balance_layers=(config.layers if config.balance_enabled else 0),
        auxk_shape=(
                ...
                if should_log:
                    ...
                    append_log_row(
                        ...
                        (
                            ...
                            *(float(value) for value in host_metrics["balance_row"]),
                            *(float(value) for value in host_metrics["auxk_row"]),
                        ),
                    )
                    console.step(
                        step_index,
    ...
    final_balance_metrics: dict[str, Any] | None = None
    if config.balance_enabled:
        balance_values = np.asarray(final_train["balance_row"], dtype=np.float32)
        metric_count = len(FUZZY_BALANCE_STAT_NAMES)
        layer_values = balance_values[metric_count:].reshape(
            (config.layers, metric_count)
        )
        final_balance_metrics = {
            ...
            "layers": [
                {
                    name: float(layer_values[layer, index])
                    for index, name in enumerate(FUZZY_BALANCE_STAT_NAMES)
                }
                for layer in range(config.layers)
            ],
        }
    final_auxk_metrics: dict[str, Any] | None = None
    if config.auxk_enabled:
        auxk_values = np.asarray(final_train["auxk_row"], dtype=np.float32)
        final_auxk_metrics = {
            "init_mode": config.init_mode,
            "mode": config.auxk_mode,
            "coefficient": config.auxk_coefficient,
            ...
            "dead_after_steps": config.dead_after_steps,
            **{
                name: float(auxk_values[index])
                for index, name in enumerate(AUXK_STAT_NAMES)
            },
        }
    ...
    result: dict[str, Any] = {
        ...
        "artifacts": {
            ...
            **(
                {"fuzzy_sparsity": FUZZY_SPARSITY_LOG_NAME} if sparsity_recorded else {}
            ),
        },
        "system": {
        ...
        "contract": {
            "model_id": "fuzzy-topk-autoencoder-gpt-v1-family",
        ...
        "metrics": {
            ...
            "fuzzy_balance": final_balance_metrics,
            "fuzzy_auxk": final_auxk_metrics,
