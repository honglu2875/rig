...
NGRAM_UPSTREAM_REPOSITORY = "https://github.com/honglu2875/ngram"
NGRAM_UPSTREAM_REVISION = "2a73b5ffbe852718dbd4e01ee6abafeb1628c5a7"
NGRAM_PATCH_PATH = (
    RECIPE_DIR
    / "patches"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    distill = parser.add_argument_group("infinigram distillation")
    distill.add_argument(
        "--ground-truth-weight",
        type=float,
        default=None,
        help="coefficient a; defaults to 1 - --infinigram-weight",
    )
    distill.add_argument(
        "--infinigram-weight",
        type=float,
    ...
    distill.add_argument(
        ...
        type=Path,
        default=None,
        help="local ngram-sa-v1 index; required when infinigram weight is positive",
    )
    distill.add_argument(
        "--infinigram-manifest",
        type=Path,
        default=DEFAULT_INFINIGRAM_MANIFEST,
        help="canonical manifest whose training split was indexed",
    )
    distill.add_argument(
        "--infinigram-max-context",
        type=positive_int,
        default=None,
        help="optional cap on teacher suffix length; default is unbounded",
    )
    distill.add_argument(
        "--infinigram-threads",
        type=positive_int,
        default=30,
        help="host query threads per JAX process",
    )
    add_standard_reporting_arguments(optim)
    return parser


def resolved_loss_weights(args: argparse.Namespace) -> tuple[float, float]:
    """Resolve a normalized two-arm objective without changing its scale."""

    infinigram_weight = float(args.infinigram_weight)
    ground_truth_weight = (
        1.0 - infinigram_weight
        if args.ground_truth_weight is None
        else float(args.ground_truth_weight)
    )
    if not math.isfinite(ground_truth_weight) or ground_truth_weight < 0.0:
        raise ValueError("--ground-truth-weight must be finite and nonnegative")
    if not math.isfinite(infinigram_weight) or infinigram_weight < 0.0:
        raise ValueError("--infinigram-weight must be finite and nonnegative")
    if not math.isclose(
        ground_truth_weight + infinigram_weight, 1.0, rel_tol=0.0, abs_tol=1.0e-12
    ):
        raise ValueError(
            "--ground-truth-weight + --infinigram-weight must equal 1 so the "
            "optimizer scale stays comparable"
        )
    return ground_truth_weight, infinigram_weight
...
def infinigram_step_seed(run_seed: int, step: int, process_index: int) -> int:
    """Stable SplitMix64 seed for one rank-local teacher batch."""

    value = (
        (int(run_seed) & _UINT64_MASK)
        ^ ((int(step) * 0xD1B54A32D192ED03) & _UINT64_MASK)
        ^ ((int(process_index) * 0x94D049BB133111EB) & _UINT64_MASK)
    )
...
class InfiniGramQueryStats:
    ...
    ground_truth_probability_sum: float = 0.0
    ...
    def add(
        ...
        self.ground_truth_probability_sum += float(
            np.sum(
                np.asarray(target_counts, dtype=np.float64)
                / np.asarray(prompt_counts, dtype=np.float64)
            )
        )
        self.query_seconds += float(seconds)


def summarize_infinigram_query_stats(
    local: InfiniGramQueryStats,
    process_count: int,
) -> dict[str, float | int]:
    values = np.asarray(
        [
            ...
            local.ground_truth_probability_sum,
            local.query_seconds,
        ],
        dtype=np.float64,
    )
    gathered = (
        np.asarray(multihost_utils.process_allgather(values), dtype=np.float64)
        if process_count > 1
        else values[None, :]
    )
...
def validate_args(
    ...
    _, infinigram_weight = resolved_loss_weights(args)
    if infinigram_weight > 0.0 and args.infinigram_index is None:
        raise ValueError(
            "--infinigram-index is required when --infinigram-weight is positive"
        )
...
def resolve_config(
    ...
    *,
    ...
    if steps > 1:
        ...
        warmup_steps = 0
    ground_truth_weight, infinigram_weight = resolved_loss_weights(args)
...
def load_infinigram_teacher(
    args: argparse.Namespace,
    config: Config,
    dataset: Any,
) -> tuple[Any | None, Mapping[str, Any] | None]:
    """Open and verify the local leave-one-out teacher when it is enabled."""

    if config.infinigram_weight == 0.0:
        return None, None
    assert args.infinigram_index is not None
    ...
    if not isinstance(files, list):
        raise ValueError(f"dataset manifest has no file inventory: {manifest_path}")
    train_entries = [
        entry
        for entry in files
        if isinstance(entry, Mapping) and entry.get("split") == "train"
    ]
    train_tokens = sum(int(entry.get("tokens", 0)) for entry in train_entries)
    train_names = [str(entry.get("path")) for entry in train_entries]
    loaded_names = [
        Path(str(getattr(shard, "filename", ""))).name for shard in dataset.train.shards
    ]
    if not train_names or loaded_names != train_names:
        raise ValueError(
            "the loaded training shard inventory does not exactly match the "
            f"infinigram manifest {manifest_path.name}"
        )
    if len(dataset.train) != train_tokens:
        raise ValueError(
            f"manifest declares {train_tokens:,} train tokens but the trainer loaded "
            f"{len(dataset.train):,}"
        )

    provenance_path = index_path / INDEX_PROVENANCE_NAME
    provenance = read_json_object(provenance_path, "infinigram index provenance")
    expected_provenance = {
        "schema_version": 1,
        "index_format": "ngram-sa-v1",
    ...
    mismatches = [
        f"{key}: expected {expected!r}, found {provenance.get(key)!r}"
        for key, expected in expected_provenance.items()
        if provenance.get(key) != expected
    ]
    if mismatches:
        raise ValueError(
            "infinigram index provenance does not match this recipe:\n  "
            + "\n  ".join(mismatches)
        )
    ...
    try:
        ...
        raise RuntimeError(
            "the treatment requires the patched ngram package; follow the recipe "
    ...
    if teacher.config.doc_sep_token != config.document_boundary_token:
        raise ValueError(
            "infinigram document separator differs from the model's boundary token"
        )
    if teacher.vocab_size > config.semantic_vocab_size:
        raise ValueError(
            f"infinigram vocabulary {teacher.vocab_size:,} exceeds model semantic "
            f"vocabulary {config.semantic_vocab_size:,}"
        )
    if teacher.tok_cnt != train_tokens:
        raise ValueError(
            f"infinigram index has {teacher.tok_cnt:,} tokens; expected {train_tokens:,}"
        )
    runtime = {
        "enabled": True,
        ...
        "dataset_manifest_sha256": manifest_sha256,
        "tokens": int(teacher.tok_cnt),
        "documents": int(teacher.doc_cnt),
        "shards": int(teacher.num_shards),
        "query_threads": int(teacher.num_threads),
...
def implementation_metadata(
    ...
    infinigram: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
...
def cross_entropy(
    ...
    config: Config,
    attention_fn: AttentionCallable | None = None,
    distill_targets: jax.Array | None = None,
) -> jax.Array:
    ...
    if config.loss_backend == "tiled":
        ...
        if config.infinigram_weight > 0.0:
            ...
            return tiled_tied_weighted_multi_cross_entropy(
                ...
                y,
                distill_targets,
                primary_weight=config.ground_truth_weight,
                distill_weight=config.infinigram_weight,
                semantic_vocab_size=config.semantic_vocab_size,
                vocab_tile_size=config.vocab_tile_size,
    ...
    selected_ground_truth = jnp.take_along_axis(
...
def _apply_training_update(
    ...
    loss, gradients = jax.value_and_grad(
        lambda candidate: cross_entropy(
            candidate, x, y, config, attention_fn, distill_targets
        )
    )(params)
    ...
    * A component whose real cost differs from its traced cost -- sparsity
    ...
    def loss(trainable: Mapping[str, Any]) -> jax.Array:
        return cross_entropy(
            trainable, tokens, targets, config, distill_targets=distill_targets
        )
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    infinigram_teacher, infinigram_runtime = load_infinigram_teacher(
        args, config, dataset
    )
    ...
    console.table(
        ...
        (
            ...
            *(
                (
                    ("infinigram tokens", f"{infinigram_teacher.tok_cnt:,}"),
                    ("infinigram threads/rank", infinigram_teacher.num_threads),
                )
    ...
    if infinigram_teacher is None:
        ...
        lowered_step = compiled_step.lower(params, optimizer, sample_x, sample_y)
    else:
        assert sample_distill is not None
        compiled_step = jax.jit(
            lambda p, o, x, y, teacher_y: train_step(
                p,
                o,
                x,
                y,
        ...
        lowered_step = compiled_step.lower(
            params, optimizer, sample_x, sample_y, sample_distill
        )
    console.phase("Compiling train step", "compilation is outside train_seconds")
    compile_started = time.perf_counter()
    executable = lowered_step.compile()
    ...
    if config.diagnostics_every:
        ...
        if infinigram_teacher is None:
            ...
            assert sample_distill is not None
            lowered_diagnostic = jax.jit(
                lambda p, o, x, y, teacher_y: diagnostic_train_step(
                    p,
                    o,
                    x,
                    y,
        ...
        diagnostic_executable = lowered_diagnostic.compile()
    ...
    sync_tree((params, optimizer, sample_x, sample_y, sample_distill, sample_mask))
    probe_detail = (
    ...
    infinigram_query_stats = InfiniGramQueryStats()
        ...
        for step_index in range(1, config.final_step + 1):
            ...
            with annotation:
                ...
                if shuffled_train_stream is None:
                    batch_x_host, batch_y_host = dataset.batch(
                        "train",
                        train_rng,
                        local_batch,
                        config.seq_len,
                    ...
                    batch_x_host, batch_y_host = shuffled_train_stream.next_batch()
                batch_distill = None
                if infinigram_teacher is not None:
                    ...
                    (
                        ...
                        max_len=config.infinigram_max_context,
                        reset_token=config.document_boundary_token,
                    )
                    query_seconds = time.perf_counter() - query_started
                    teacher_targets = np.ascontiguousarray(
                        teacher_targets, dtype=np.int32
                    )
                    infinigram_query_stats.add(
                        teacher_targets,
                        batch_y_host,
                        suffix_lengths,
                        prompt_counts,
                ...
                batch_x = put_host_local_array(
                    batch_x_host, mesh, P("data", None), data_sharding, process_count
                )
                batch_y = put_host_local_array(
                    batch_y_host, mesh, P("data", None), data_sharding, process_count
                )
                if should_run_diagnostics(
                    ...
                    if batch_distill is None:
                        ...
                        (
                            ...
                            params, optimizer, batch_x, batch_y, batch_distill
                        )
    ...
    infinigram_query_summary = (
        summarize_infinigram_query_stats(infinigram_query_stats, process_count)
        if infinigram_teacher is not None
        else None
    )
        ...
        console.table(
            ...
            (
                ...
                *(
                    (
                        (
                            "infinigram query rate",
                            f"{format_rate(float(infinigram_query_summary['global_query_tokens_per_second']))} tok/s",
                        ),
                    )
                    if infinigram_query_summary is not None
                    else ()
                ),
    ...
    result: dict[str, Any] = {
        ...
        "implementation": implementation_metadata(
            config, attention_runtime, infinigram_runtime
        ),
