...
) -> jax.Array | tuple[jax.Array, jax.Array] | tuple[jax.Array, FuzzyBalanceStats]:
    """Return final normalized token representations before the tied head."""

    supplied = sum(
        operation is not None
        for operation in (sparse_mlp_fn, sparse_mlp_diagnostic_fn, balanced_mlp_fn)
    )
    if supplied > 1:
        raise ValueError(
            "ordinary, diagnostic, and balanced MLP callables are exclusive"
        )
...
def cross_entropy_and_balance(
    ...
    sparse_mlp_fn: FuzzyTopKCallable | None = None,
    balanced_mlp_fn: FuzzyTopKBalanceCallable | None = None,
) -> tuple[jax.Array, tuple[jax.Array, FuzzyBalanceStats | None]]:
    """Training objective plus unpolluted CE and per-block balance metrics."""

    if not config.balance_enabled:
        cross_entropy_loss = cross_entropy(
            params, x, y, config, attention_fn, sparse_mlp_fn
        )
        return cross_entropy_loss, (cross_entropy_loss, None)
    if balanced_mlp_fn is None:
        raise ValueError("enabled fuzzy homeostasis has no balanced MLP callable")

    hidden_and_balance = gpt_hidden(
        params,
        x,
        config,
        attention_fn,
    ...
    hidden, balance = hidden_and_balance
    if not isinstance(balance, FuzzyBalanceStats):
    ...
    return objective, (cross_entropy_loss, balance)
...
def fuzzy_training_log_columns(config: Config) -> tuple[logpack.Column, ...]:
    ...
    for layer in range(config.layers):
        columns.extend(
            logpack.column(name, "block", layer) for name in FUZZY_BALANCE_STAT_NAMES
        )
    return tuple(columns)


def init_optimizer(
    params: Any, steps: int, *, balance_layers: int = 0
) -> dict[str, Any]:
    ...
    history = np.zeros((steps, 3 + balance_row_width(balance_layers)), dtype=np.float32)
    return {
...
def _apply_training_update(
    ...
    objective, (loss, balance) = objective_and_aux
    ...
    balance_values = balance_row(balance)
    history_row = jnp.concatenate(
        (jnp.stack((loss, lr, grad_norm)).astype(jnp.float32), balance_values)
    )
