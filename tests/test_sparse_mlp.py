...
class SparseMlpKernelTests(unittest.TestCase):
    ...
    def inputs(dtype=jnp.float32):
        ...
        up_weight = (
            jax.random.normal(jax.random.key(2), (128, 256), dtype=dtype) * 0.05
        )
        up_bias = (
            jax.random.normal(jax.random.key(3), (256,), dtype=dtype) * 0.01
        )
        down_weight = (
            jax.random.normal(jax.random.key(4), (256, 128), dtype=dtype) * 0.05
        )
        down_bias = (
            jax.random.normal(jax.random.key(5), (128,), dtype=dtype) * 0.01
        )
    ...
    def test_pallas_interpreter_matches_selected_row_oracle_with_padding(
        ...
        expected = reference_sparse_decode(
            values, indices, down_weight, down_bias
        )
    ...
    def test_fused_forward_and_every_gradient_match_dense_topk_dense(
        ...
        def actual_loss(*operands):
            return jnp.sum(
                sparse_topk_mlp(*operands, config=config) * cotangent
            )

        def expected_loss(*operands):
            return jnp.sum(
                naive_dense_topk_mlp(*operands, top_k=8) * cotangent
            )
